...
const Collection = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await serialFetch(`${SCRIPT_URL}?sheet=${SHEET_NAME}&action=stageSplit&presenceCol=${encodeURIComponent('Planned 10')}&completeCol=${encodeURIComponent('Actual 10')}`);
      const result = await response.json();
      if (result.success) {
        const headers = result.headers || [];
        setIndents([{ rowData: headers, originalIndex: -1 }, ...filterRowsByFirmAccess(result.pending || [], headers, '/collection')]);
        setHistoryIndents([{ rowData: headers, originalIndex: -1 }, ...filterRowsByFirmAccess(result.history || [], headers, '/collection')]);
      } else {
        setIndents([]);
        setHistoryIndents([]);
