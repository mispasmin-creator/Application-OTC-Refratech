...
const ActualWorkDone = () => {
  ...
  const [headers, setHeaders] = useState(ACTUAL_WORK_HEADERS);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await serialFetch(`${SCRIPT_URL}?sheet=${encodeURIComponent(ACTUAL_WORK_SHEET)}`);
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        if (result.data.length > 5) {
          const row5Matches = result.data[5].some(h => ['timestamp', 'application no.', 'serial number', 'date', 'site incharge'].includes(cleanH(h)));
      ...
      console.error('Error fetching Actual Work Done data:', err);
      setHeaders(ACTUAL_WORK_HEADERS);
      setRows([]);
    } finally {
      setFetching(false);
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '3rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>Actual Work Done</h1>
          <p style={{ color: 'var(--text-muted)' }}>Records of daily actual work progress, contractor details, labours, and product quantities.</p>
        </div>
      </div>
      ...
      <div className="table-container">
        <div style={{ position: 'relative', width: '100%', maxWidth: '100%', marginBottom: '1.5rem', display: 'flex' }}>
          ...
          <input
            type="text"
            placeholder="Search by Application No., Contractor, Product, Incharge, or any keyword..."
            value={searchQuery}
        ...
        {fetching ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <p>Loading Actual Work Done records...</p>
          </div>
        ) : (
          <div>
                ...
                {filteredRows.length === 0 ? (
                  <tr>
                    <td colSpan={headers.length} style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                      No actual work done records found.
                    </td>
                  </tr>
                ) : (
