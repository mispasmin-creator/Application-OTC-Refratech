...
const CastingInspection = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        const planned5Idx = findIdx('Planned 5');
        const actual5Idx = findIdx('Actual 5');
        ...
        const filteredRows = allMapped.filter(item => {
          ...
          const hasPlanned5 = planned5Idx !== -1 && row[planned5Idx] !== undefined && row[planned5Idx] !== null && row[planned5Idx].toString().trim() !== '';
          const noActual5 = actual5Idx === -1 || row[actual5Idx] === undefined || row[actual5Idx] === null || row[actual5Idx].toString().trim() !== '';
          return hasPlanned5 && noActual5;
        });

        const historyRows = allMapped.filter(item => {
          const row = item.rowData;
          const hasActual5 = actual5Idx !== -1 && row[actual5Idx] !== undefined && row[actual5Idx] !== null && row[actual5Idx].toString().trim() !== '';
          return hasActual5;
        });
  ...
  const handleSubmit = async () => {
    ...
    const status5Idx = findIdx('Status 5');
    const actual5Idx = findIdx('Actual 5');
    ...
    try {
      ...
      if (allSuccess) {
        setMessage({ type: 'success', text: 'Casting Inspection details updated successfully!' });
        setShowModal(false);
      ...
      <div className="glass-panel" style={{ padding: '2rem', borderRadius: '12px', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <h2 style={{ fontSize: '1.25rem' }}>
            {activeTab === 'pending' ? 'Pending Casting Inspection' : 'History Records'}
          </h2>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div className="glass-panel animate-fade-in" style={{ padding: '2rem', borderRadius: '12px', minWidth: '400px', maxWidth: '90%', border: '1px solid var(--border-color)' }}>
            ...
            <div style={{ marginBottom: '2rem' }}>
              <label className="form-label">Status 5</label>
              <select className="form-input" value={status5} onChange={(e) => setStatus5(e.target.value)} style={{ backgroundColor: 'rgba(15, 23, 42, 0.9)' }}>
