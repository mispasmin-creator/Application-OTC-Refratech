...
const SHEET_NAME = 'Received At Site';

const RECEIVED_AT_SITE_HEADERS = [
  'Timestamp',
  'Application No.',
  'Serial Number',
  'Firm',
...
const ReceivedAtSite = () => {
  const [rows, setRows] = useState([]);
  const [headers, setHeaders] = useState(RECEIVED_AT_SITE_HEADERS);
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        if (result.data.length > 5) {
          const row5Matches = result.data[5].some(h => ['timestamp', 'application no.', 'serial number', 'product name'].includes(cleanH(h)));
        ...
        setRows(filterRowsByFirmAccess(dataRows, validHeaders, '/received-at-site'));
      } else {
        setHeaders(RECEIVED_AT_SITE_HEADERS);
        setRows([]);
      }
    } catch (err) {
      console.error('Error fetching Received At Site data:', err);
      setHeaders(RECEIVED_AT_SITE_HEADERS);
      setRows([]);
    } finally {
      setFetching(false);
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '3rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>Received At Site</h1>
          <p style={{ color: 'var(--text-muted)' }}>Records of materials dispatched and received at site with supervisor details.</p>
        </div>
      </div>
      ...
      <div className="table-container">
        <div style={{ position: 'relative', width: '100%', maxWidth: '100%', marginBottom: '1.5rem', display: 'flex' }}>
          ...
          <input
            type="text"
            placeholder="Search by Application No., Serial No., Product, Party Name, or any keyword..."
            value={searchQuery}
        ...
        {fetching ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <p>Loading site receipt records...</p>
          </div>
        ) : (
          <div>
