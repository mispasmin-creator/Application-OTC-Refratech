...
const TakeQtyConfirmation = () => {
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        const filteredRows = allMapped.filter(item => {
          ...
          const hasPlanned8 = planned8Idx !== -1 && row[planned8Idx] !== undefined && row[planned8Idx] !== null && row[planned8Idx].toString().trim() !== '';
          const noActual8 = actual8Idx === -1 || row[actual8Idx] === undefined || row[actual8Idx] === null || row[actual8Idx].toString().trim() !== '';
          return hasPlanned8 && noActual8;
        });

        const historyRows = allMapped.filter(item => {
          const row = item.rowData;
          const hasActual8 = actual8Idx !== -1 && row[actual8Idx] !== undefined && row[actual8Idx] !== null && row[actual8Idx].toString().trim() !== '';
          return hasActual8;
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (allSuccess) {
        setMessage({ type: 'success', text: 'Take Qty Confirmation details updated successfully!' });
        setShowModal(false);
      ...
      <div className="glass-panel" style={{ padding: '2rem', borderRadius: '12px', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <h2 style={{ fontSize: '1.25rem' }}>
            {activeTab === 'pending' ? 'Pending Take Qty Confirmation' : 'History Records'}
          </h2>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div className="glass-panel animate-fade-in" style={{ padding: '2rem', borderRadius: '12px', minWidth: '400px', maxWidth: '90%', border: '1px solid var(--border-color)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <select className="form-input" value={status8} onChange={(e) => setStatus8(e.target.value)} style={{ backgroundColor: 'rgba(15, 23, 42, 0.9)' }}>
