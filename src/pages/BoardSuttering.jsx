...
const BoardSuttering = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        const planned4Idx = findIdx('Planned 4');
        const actual4Idx = findIdx('Actual 4');
        ...
        const filteredRows = allMapped.filter(item => {
          ...
          const hasPlanned4 = planned4Idx !== -1 && row[planned4Idx] !== undefined && row[planned4Idx] !== null && row[planned4Idx].toString().trim() !== '';
          const noActual4 = actual4Idx === -1 || row[actual4Idx] === undefined || row[actual4Idx] === null || row[actual4Idx].toString().trim() === '';
          return hasPlanned4 && noActual4;
        });

        const historyRows = allMapped.filter(item => {
          const row = item.rowData;
          const hasActual4 = actual4Idx !== -1 && row[actual4Idx] !== undefined && row[actual4Idx] !== null && row[actual4Idx].toString().trim() !== '';
          return hasActual4;
        });
  ...
  const handleSubmit = async () => {
    ...
    const status4Idx = findIdx('Status 4');
    const actual4Idx = findIdx('Actual 4');
    ...
    try {
      ...
      if (allSuccess) {
        setMessage({ type: 'success', text: 'Board Suttering details updated successfully!' });
        setShowModal(false);
  ...
  return (
    <div className="animate-fade-in" style={{ paddingBottom: '3rem', position: 'relative' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>Board Suttering</h1>
          <p style={{ color: 'var(--text-muted)' }}>Manage records pending Board Suttering.</p>
        </div>
      ...
      <div className="glass-panel" style={{ padding: '2rem', borderRadius: '12px', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <h2 style={{ fontSize: '1.25rem' }}>
            {activeTab === 'pending' ? 'Pending Board Suttering' : 'History Records'}
          </h2>
                  ...
                  {rows.length === 0 ? (
                    <tr>
                      <td colSpan={columnsToRender.length + (activeTab === 'pending' ? 1 : 0)} style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                        {activeTab === 'pending' ? 'No records pending Board Suttering.' : 'No history records found.'}
                      </td>
                    </tr>
                  ) : (
                    rows.map((item, index) => (
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div className="glass-panel animate-fade-in" style={{ padding: '2rem', borderRadius: '12px', minWidth: '400px', maxWidth: '90%', border: '1px solid var(--border-color)' }}>
            ...
            <div style={{ marginBottom: '2rem' }}>
              <label className="form-label">Status 4</label>
              <select className="form-input" value={status4} onChange={(e) => setStatus4(e.target.value)} style={{ backgroundColor: 'rgba(15, 23, 42, 0.9)' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn-primary" onClick={handleSubmit} disabled={submitting}>
                {submitting ? <Loader2 size={16} className="animate-spin" style={{ animation: 'spin 1s linear infinite' }} /> : 'Submit Board Suttering'}
              </button>
            </div>
          </div>
        </div>
