...
const SettleAccountSupervisor = () => {
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        const filteredRows = allMapped.filter(item => {
          ...
          const hasPlanned11 = planned11Idx !== -1 && row[planned11Idx] !== undefined && row[planned11Idx] !== null && row[planned11Idx].toString().trim() !== '';
          const noActual11 = actual11Idx === -1 || row[actual11Idx] === undefined || row[actual11Idx] === null || row[actual11Idx].toString().trim() !== '';
          return hasPlanned11 && noActual11;
        });

        const historyRows = allMapped.filter(item => {
          const row = item.rowData;
          const hasActual11 = actual11Idx !== -1 && row[actual11Idx] !== undefined && row[actual11Idx] !== null && row[actual11Idx].toString().trim() !== '';
          return hasActual11;
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (allSuccess) {
        setMessage({ type: 'success', text: 'Supervisor account settlement details updated successfully!' });
        setShowModal(false);
      ...
      <div className="glass-panel" style={{ padding: '2rem', borderRadius: '12px', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <h2 style={{ fontSize: '1.25rem' }}>
            {activeTab === 'pending' ? 'Pending Settlements' : 'History Records'}
          </h2>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div className="glass-panel animate-fade-in" style={{ padding: '2rem', borderRadius: '12px', minWidth: '400px', maxWidth: '90%', border: '1px solid var(--border-color)' }}>
            ...
            <div style={{ marginBottom: '2rem' }}>
              ...
              <select className="form-input" value={status11} onChange={(e) => setStatus11(e.target.value)} style={{ backgroundColor: 'rgba(15, 23, 42, 0.9)' }}>
